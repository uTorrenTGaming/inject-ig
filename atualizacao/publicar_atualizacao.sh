...
# 2. Pergunta sobre o changelog antes de atualizar a versão
echo -e "${CYAN}Quais as melhorias e updates desta versão? (Pressione ENTER para pular)${NC}"
read -r UPDATE_MSG

if [ -n "$UPDATE_MSG" ]; then
    echo -e "${GREEN}[+] Salvando changelog no banco de dados para os clientes...${NC}"
    node ../atualizacao/post_update.js "$UPDATE_MSG"
fi

# 3. Atualiza a versão no package.json automaticamente (ex: 2.0.1 -> 2.0.2)
echo -e "\n${GREEN}[1/5] 📦 Atualizando versão...${NC}"
...
git add ../atualizacao/publicar_atualizacao.sh 2>/dev/null || true
git commit -m "Deploy automático de atualização: $NEW_VERSION" || true
git -c credential.helper= push https://$GH_TOKEN@github.com/uTorrenTGaming/inject-ig.git main
