...
// ── SUPPORT TICKETS & APP UPDATES IPC ──
ipcMain.handle('admin.getAllActiveChats', async () => {
    try {
        const { data, error } = await supabase
...
ipcMain.handle('admin.getSupportMessages', async (event, hwid) => {
    try {
        const { data, error } = await supabase
            .from('support_tickets')
            .select('*')
...
ipcMain.handle('admin.submitSupportTicket', async (event, hwid, message, isAdmin = true) => {
    try {
        const { error } = await supabase
            .from('support_tickets')
...
ipcMain.handle('admin.deleteSupportTicket', async (event, id) => {
    try {
        const { error } = await supabase
            .from('support_tickets')
            .delete()
...
ipcMain.handle('admin.postAdminMessage', async (event, content) => {
    try {
        const { error } = await supabase
            .from('app_updates')
            .insert([{
...
// Setup Realtime Listener for Support Tickets
function setupRealtimeListener() {
    if (!supabase) return;
    supabase.channel('admin-support-realtime')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'support_tickets' }, (payload) => {
          if (mainWindow && !mainWindow.isDestroyed()) {
              mainWindow.webContents.send('realtime-support-message', payload.new);
          }
      })
      .subscribe();
}
