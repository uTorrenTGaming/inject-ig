...
(function() {
    ...
    if (window.adminAPI) {
        ...
        console.log("[UniversalAPI] Usando ambiente nativo do Electron com fallbacks.");
    }
    ...
    const fallbacks = {
        ...
        getPendingPayments: async () => {
            try {
                // Fetch inactive licenses with PENDING| prefix
                const data = await supabaseFetch('licenses', 'GET', null, '?is_active=eq.false&key=like.PENDING|*&order=created_at.desc');
                if (!data) return [];
        ...
        approvePayment: async (id) => {
            try {
                const data = await supabaseFetch('licenses', 'GET', null, `?id=eq.${id}`);
                if (!data || data.length === 0) return { success: false, message: 'Pagamento não encontrado' };
                
                const pending = data[0];
                if (!pending.key.startsWith('PENDING|')) return { success: false, message: 'Não é um pagamento pendente' };

                const key = 'IG-' + Math.random().toString(36).substring(2, 6).toUpperCase() + '-' + 
                            Math.random().toString(36).substring(2, 6).toUpperCase() + '-' + 
                            Math.random().toString(36).substring(2, 6).toUpperCase();
                
                await supabaseFetch('licenses', 'PATCH', { 
                    key: key,
                    is_active: true,
        ...
        getSupportTickets: async () => {
            try {
                const data = await supabaseFetch('support_tickets', 'GET', null, '?order=created_at.desc');
                return data || [];
            } catch (e) { return []; }
        },
        deleteSupportTicket: async (id) => {
            try {
                await supabaseFetch('support_tickets', 'DELETE', null, `?id=eq.${id}`);
                return { success: true };
            } catch (e) {
                return { success: false, message: e.message };
            }
        },
        postAdminMessage: async (content) => {
            try {
                await supabaseFetch('app_updates', 'POST', {
                    type: 'message',
                    version: 'Admin',
                    content: content
                });
    ...
    window.universalAPI = Object.assign({}, fallbacks, adminAPI || {});
})();
