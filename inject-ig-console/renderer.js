...
// ═══════════ Audio UX Engine (Convertido para Vibracao) ═══════════
...
// Dynamic Island Engine extracted to assets/dynamic-island.js
...
segBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        // Update macOS Titlebar
        const macTitlebar = document.getElementById('mac-titlebar');
        if (macTitlebar && targetId !== 'view-splash' && !targetId.startsWith('auth')) {
            const navIconEl = btn.querySelector('.nav-icon');
            const iconHtml = navIconEl ? navIconEl.innerHTML.replace('width="14" height="14"', 'width="20" height="20"') : '';
            const iconBg = navIconEl ? navIconEl.style.background : 'transparent';
            const btnText = btn.textContent.trim().replace('•', '').trim();
            macTitlebar.innerHTML = `<div style="width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center; background: ${iconBg}; margin-right: 8px;">${iconHtml}</div><span>${btnText}</span>`;
        }
        
        // --- Dynamic Island Menu Animation ---
        if (window.DI_Engine) {
            if (targetId === 'view-radar') window.DI_Engine.showRadar('#22c55e');
            else if (targetId === 'view-inject') window.DI_Engine.showInject('#ef4444');
            else if (targetId === 'view-dashboard') window.DI_Engine.showDashboard('#3b82f6');
            else {
                const btnText = btn.textContent.trim().replace('•', '').trim();
                let color = '#3b82f6';
                let iconSvg = '';
                switch(targetId) {
                    case 'view-terminal': iconSvg = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#a855f7" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="filter: drop-shadow(0 0 4px rgba(168, 85, 247, 0.4));"><polyline points="4 17 10 11 4 5"></polyline><line x1="12" y1="19" x2="20" y2="19"></line></svg>'; color = '#a855f7'; break;
                    case 'view-vault': iconSvg = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#eab308" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="filter: drop-shadow(0 0 4px rgba(234, 179, 8, 0.4));"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>'; color = '#eab308'; break;
                    case 'view-osint': iconSvg = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#0ea5e9" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="filter: drop-shadow(0 0 4px rgba(14, 165, 233, 0.4));"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>'; color = '#0ea5e9'; break;
                    case 'view-spectre': iconSvg = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#f43f5e" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="filter: drop-shadow(0 0 4px rgba(244, 63, 94, 0.4));"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>'; color = '#f43f5e'; break;
                }
                window.DI_Engine.showDefault(btnText, color, iconSvg);
            }
        }
...
document.getElementById('tab-files')?.addEventListener('click', () => {
    showSubPane('tab-files');
    // Re-render sempre que o usuário clica na aba
    if (Object.keys(currentVaultData).length > 0) {
...
function renderModuleRows(data) {
    const row = (k, v, cls = '') => `<div class="row ${cls}">
        <span class="row-k">${k}</span>
        <span class="row-v">${v}</span>
    </div>`;
    if (!data) return '';
...
// ═══════════════════════════════════════════════════════════════════
// ── SITE EXTRACTOR ENGINE — Motor Java Autônomo na porta 7890 ─────
// ═══════════════════════════════════════════════════════════════════

let extractorPollInterval = null;
let currentExtractorData = {};
let activeCategory = 'js';
let scanHistory = [];
...
function addToHistory(url, summary) {
    scanHistory.unshift({ url, summary, ts: new Date().toLocaleTimeString() });
    if (scanHistory.length > 10) scanHistory.pop();
    const histEl = document.getElementById('vault-history');
    if (!histEl) return;
    histEl.innerHTML = scanHistory.map(h => `
        <div style="padding:4px 0; border-bottom:1px solid var(--border-1); font-size:10px;">
            <div style="color:var(--text-1); white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${h.url}</div>
            <div style="color:var(--text-3); font-family:var(--mono);">${h.ts} · ${h.summary?.totalItems || 0} itens</div>
        </div>
    `).join('');
}

function renderExtractorSummary(data) {
    ...
    results.innerHTML = `
        <div style="display:flex; flex-direction:column; gap:8px;">
            ...
            <div style="background:rgba(255,255,255,0.03); border:1px solid var(--border-1); border-radius:8px; padding:10px;">
                ...
                ${meta.generator ? `<div style="margin-top:4px;"><span class="tag tag-blue" style="font-size:9px;">${meta.generator}</span></div>` : ''}
            </div>

            <!-- Stats Grid -->
            <div style="display:grid; grid-template-columns:repeat(3,1fr); gap:5px;">
                ${Object.entries(sum).filter(([k]) => k.endsWith('Count') && k !== 'totalItems').map(([k, v]) => {
...
function switchToFilesCategory(catKey) {
    showSubPane('tab-files');
    document.querySelectorAll('#view-vault .sub-seg-btn').forEach(b => {
        if (b.getAttribute('data-pane') === 'pane-files') b.classList.add('active');
        else b.classList.remove('active');
    });
    activeCategory = catKey;
    renderExtractorFiles(currentExtractorData, catKey);
}

function renderExtractorFiles(data, forceCategory = null) {
    const categoryTabsEl = document.getElementById('files-category-tabs');
    const mainList = document.getElementById('main-files-list');
    const counterEl = document.getElementById('files-counter');
    if (!mainList) return;

    const availableCats = Object.keys(EXTRACTOR_CATEGORIES).filter(k =>
        k !== 'meta' && k !== 'summary' && Array.isArray(data[k]) && data[k].length > 0
    );

    if (availableCats.length === 0) {
        mainList.innerHTML = '<div style="text-align:center;color:var(--text-3);font-size:10px;padding:20px 0;">Execute uma varredura primeiro</div>';
        if (counterEl) counterEl.textContent = '0 itens';
        if (categoryTabsEl) categoryTabsEl.innerHTML = '';
        return;
    }

    // Set active category
    if (forceCategory && availableCats.includes(forceCategory)) {
        activeCategory = forceCategory;
    } else if (!availableCats.includes(activeCategory)) {
        activeCategory = availableCats[0];
    }

    // Render category tabs
    if (categoryTabsEl) {
        categoryTabsEl.innerHTML = availableCats.map(cat => {
            const cfg = EXTRACTOR_CATEGORIES[cat] || {};
            const count = (data[cat] || []).length;
            const isActive = cat === activeCategory;
            return `<button onclick="window._extSwitchCat('${cat}')" style="
                padding: 4px 10px; border-radius:20px; border:1px solid ${isActive ? cfg.color || 'var(--accent)' : 'var(--border-1)'};
                background:${isActive ? (cfg.color || 'var(--accent)') + '22' : 'rgba(255,255,255,0.03)'};
                color:${isActive ? (cfg.color || 'var(--accent)') : 'var(--text-3)'}; font-size:10px; cursor:pointer;
                transition:all .2s; white-space:nowrap; font-family:var(--mono);">
                ${cfg.emoji || ''} ${cfg.label || cat} <span style="font-weight:700;">${count}</span>
            </button>`;
        }).join('');
    }

    // Switch category callback
    window._extSwitchCat = (cat) => {
        activeCategory = cat;
        renderExtractorFiles(currentExtractorData);
    };
    ...
    const cfg = EXTRACTOR_CATEGORIES[activeCategory] || {};

    const filteredItems = searchQuery
        ? items.filter(item => JSON.stringify(item).toLowerCase().includes(searchQuery))
        : items;

    if (counterEl) counterEl.textContent = `${filteredItems.length} itens em ${cfg.label || activeCategory}`;

    mainList.innerHTML = '';
    if (filteredItems.length === 0) {
        mainList.innerHTML = '<div style="text-align:center;color:var(--text-3);font-size:10px;padding:20px 0;">Nenhum item nesta categoria</div>';
        return;
    }
    ...
    filteredItems.forEach((item, idx) => {
        ...
        el.style.cssText = `
            ...
            border-radius:8px; cursor:default; transition:all .15s;
            margin-bottom:2px;
        `;
        el.onmouseenter = () => {
            el.style.background = 'rgba(255,255,255,0.05)';
            el.style.borderColor = cfg.color || 'var(--accent)';
        };
        el.onmouseleave = () => {
        ...
        contentEl.style.cssText = `flex:1; overflow:hidden; min-width:0;`;

        const primaryText = item.url || item.endpoint || item.name || item.filename || item.keyword || item.directive || JSON.stringify(item).slice(0, 80);
        const secondaryText = buildSecondaryText(item, activeCategory);

        contentEl.innerHTML = `
            <div style="font-size:10px; font-family:var(--mono); color:var(--text-1);
                white-space:nowrap; overflow:hidden; text-overflow:ellipsis;"
                title="${escapeHtml(String(primaryText))}">${escapeHtml(String(primaryText))}</div>
            ${secondaryText ? `<div style="font-size:9px; color:var(--text-3); margin-top:1px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${escapeHtml(secondaryText)}</div>` : ''}
        `;

        // Tags extras
        const tagsEl = document.createElement('div');
        tagsEl.style.cssText = `display:flex; gap:3px; flex-shrink:0;`;

        // Badge de tipo
        if (item.type || item.format || item.platform) {
            const typeTag = document.createElement('span');
            typeTag.className = 'tag';
            typeTag.style.fontSize = '8px';
            typeTag.textContent = item.type || item.format || item.platform;
            tagsEl.appendChild(typeTag);
        }
        if (item.isExternal) {
            const extTag = document.createElement('span');
            extTag.className = 'tag tag-red';
            extTag.style.fontSize = '8px';
            extTag.textContent = 'EXT';
            tagsEl.appendChild(extTag);
        }
        if (item.hasEval) {
            const evalTag = document.createElement('span');
            evalTag.className = 'tag tag-red';
            evalTag.style.fontSize = '8px';
            evalTag.textContent = 'eval()';
            tagsEl.appendChild(evalTag);
        }

        // Botão de download individual JSON
        const dlBtn = document.createElement('button');
        dlBtn.className = 'btn btn-ghost';
        dlBtn.style.cssText = `flex:0 0 auto; padding:4px 8px; font-size:9px; white-space:nowrap; color:${cfg.color || 'var(--accent)'};`;
...
function buildSecondaryText(item, category) {
    switch (category) {
        case 'js':
        case 'css':
        case 'fonts':     return item.filename || '';
        case 'images':    return item.alt ? `alt: ${item.alt}` : item.filename || '';
        case 'links':     return item.text || '';
        case 'endpoints': return `padrão: ${item.pattern || ''}`;
        case 'forms':     return `${item.method?.toUpperCase() || 'GET'} → ${item.action || '/'} · ${item.fieldCount || 0} campos`;
        case 'scripts':   return `${item.length || 0} chars${item.hasFetch ? ' · fetch' : ''}${item.hasXHR ? ' · xhr' : ''}${item.hasWebSocket ? ' · ws' : ''}`;
        case 'cookies':   return item.source || '';
        case 'database':  return `${item.occurrences || 0} ocorrência(s)`;
        case 'headers':   return item.value ? item.value.slice(0, 60) : '';
        case 'sitemap':   return `via ${item.source || ''}`;
        case 'robots':    return item.value || '';
        case 'videos':    return item.videoId ? `YouTube: ${item.videoId}` : item.filename || '';
        case 'iframes':   return '';
        default:          return '';
    }
}
...
async function pollExtractorResult() {
    try {
        const res = await window.electronAPI.extractorResult();
        if (!res || !res.success || !res.data) return;
        const data = res.data;
        if (data.status !== 'complete') return;

        currentExtractorData = data;
        clearInterval(extractorPollInterval);
        extractorPollInterval = null;

        // Update UI
        setExtractorStatus(`✅ Scan completo — ${data.summary?.totalItems || 0} itens extraídos`);
        renderExtractorSummary(data);
        renderExtractorFiles(data);
        addToHistory(data.target, data.summary);
        updateExtractorTelemetryUI(data);

        // Update monitor live feed
        appendMonitorLine(`[EXTRACTOR] Scan completo: ${data.target}`);
        appendMonitorLine(`[EXTRACTOR] ${data.summary?.totalItems || 0} itens — JS:${data.summary?.jsCount||0} CSS:${data.summary?.cssCount||0} IMG:${data.summary?.imagesCount||0}`);

        // Update telemetry stats
        const telVault = document.getElementById('telemetry-vault');
        if (telVault) telVault.textContent = data.summary?.totalItems || 0;
        const telIntercepts = document.getElementById('telemetry-intercepts');
        if (telIntercepts) telIntercepts.textContent = data.target || '-';

        document.getElementById('extractor-status-dot')?.style && (document.getElementById('extractor-status-dot').style.background = 'var(--green)');
        document.getElementById('extractor-status-label') && (document.getElementById('extractor-status-label').textContent = 'Online');
        document.getElementById('extractor-status-label') && (document.getElementById('extractor-status-label').className = 'tag tag-green');
    } catch (e) {
        // silently ignore poll errors
    }
}

function updateExtractorTelemetryUI(data) {
    const meta = data.meta || {};
    const engDetail = document.getElementById('extractor-engine-detail');
    if (engDetail) engDetail.textContent = `Porta 7890 · Target: ${meta.host || data.target || '-'}`;
}

// ── Botão Varrer (Motor Extractor) ──
document.getElementById('btn-run-scan')?.addEventListener('click', async () => {
    ...
    // DI animation
    window.DI_Engine?.showAction?.('Extraindo Site', `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#22c55e" stroke-width="2.5"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>`, 8000);

    setExtractorStatus(`⏳ Iniciando motor Java...`, true);

    // Clear old data
    const results = document.getElementById('vault-results');
    if (results) results.innerHTML = '<div style="text-align:center;color:var(--text-3);font-size:10px;padding:20px 0;font-family:var(--mono);">Iniciando varredura...</div>';
    const mainList = document.getElementById('main-files-list');
    if (mainList) mainList.innerHTML = '<div style="text-align:center;color:var(--text-3);font-size:10px;padding:20px 0;">Varrendo...</div>';

    try {
        // Envia para o motor Java
        const res = await window.electronAPI.extractorScan(url);
        if (!res.success) {
            setExtractorStatus(`❌ Erro: ${res.message}`);
            return;
        }

        setExtractorStatus(`🔄 Varrendo ${url} — aguardando resultados...`, true);
        appendMonitorLine(`[EXTRACTOR] Iniciando scan: ${url}`);
        term.writeln(`\r\n\x1b[33m[EXTRACTOR]\x1b[0m Varrendo: ${url}`);

        // Poll a cada 1.5s até ter resultado
        clearInterval(extractorPollInterval);
        extractorPollInterval = setInterval(pollExtractorResult, 1500);
        ...
        setExtractorStatus(`❌ ${err.message}`);
        term.writeln(`\r\n\x1b[31m[EXTRACTOR ERROR]\x1b[0m ${err.message}`);
    }
});
...
document.getElementById('btn-run-scan-local')?.addEventListener('click', async () => {
    try {
        ...
        if (window.DI_Engine && window.DI_Engine.showAction) {
            window.DI_Engine && window.DI_Engine.showAction('Lendo Arquivos', '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#eab308" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="filter: drop-shadow(0 0 4px rgba(234, 179, 8, 0.4));"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>', 4000);
        }
...
function renderFileTree(data) {
    ...
    const addFolder = (title, items, color) => {
        let validItems = [];
        if (Array.isArray(items)) validItems = items;
        else if (typeof items === 'object' && items !== null) {
            validItems = Object.entries(items).map(([k, v]) => `${k}: ${typeof v === 'object' ? JSON.stringify(v) : v}`);
        }
        ...
        f.className = 'folder';
        f.innerHTML = `<div class="folder-dot" style="background:${COLORS_MAP[color] || COLORS_MAP.blue}"></div><span style="flex: 1;">${title} <span style="color:var(--text-3)">${validItems.length}</span></span><button class="btn btn-ghost" style="width: auto; padding: 2px 8px; font-size: 9px; margin-right: 8px;">Baixar Lista</button>`;
        ...
        f.querySelector('button').onclick = (e) => {
            e.stopPropagation();
            const contentToSave = validItems.map(i => typeof i === 'object' ? JSON.stringify(i) : String(i)).join('\\n');
            downloadData(`${title}.txt`, contentToSave);
        };

        validItems.forEach(item => {
            const val = typeof item === 'object' ? JSON.stringify(item) : String(item);
            const el = document.createElement('div');
            el.className = 'folder-child';
        ...
        f.onclick = (e) => {
            if (e.target.tagName === 'BUTTON') return; // Não propagar clique no botão
    ...
    addFolder('Emails Encontrados', h.emails || data.emails, 'amber');
    addFolder('Chaves / Tokens', h.keys || data.keys, 'red');
    addFolder('Endpoints', data.architecture?.form_endpoints || data.api_endpoints, 'blue');
    addFolder('Scripts', data.architecture?.scripts || data.inline_scripts, 'gray');
    addFolder('Links Internos', data.architecture?.links || data.assets, 'gray');
    addFolder('Redes Sociais', dr.social_links || data.social_links, 'blue');
    addFolder('Telefones', dr.phone_numbers || data.phone_numbers, 'green');
    addFolder('Subdomínios', data.subdomains, 'amber');

    // Dynamic Fallback for everything else
    const explicitKeys = ['exposed_paths', 'emails', 'keys', 'form_endpoints', 'api_endpoints', 'scripts', 'inline_scripts', 'links', 'assets', 'social_links', 'phone_numbers', 'subdomains'];
    
    for (const key of Object.keys(data)) {
        if (['target', 'harvested_data', 'deep_recon', 'architecture'].includes(key) || explicitKeys.includes(key)) continue;
        
        const val = data[key];
        if ((Array.isArray(val) && val.length > 0) || (typeof val === 'object' && val !== null && Object.keys(val).length > 0)) {
             const meta = MODULE_META[key] || { label: key.replace(/_/g, ' '), color: 'gray' };
             addFolder(meta.label, val, meta.color || 'gray');
        }
    }
    // Se nada foi renderizado, mostrar estado vazio informativo
    if (mainList.children.length === 0) {
        mainList.innerHTML = '<div style="text-align: center; color: var(--text-3); font-size: 11px; padding: 30px 10px; line-height: 1.6;">Nenhum dado de arquivo encontrado neste scan.<br>Tente varrer um site com recursos públicos.</div>';
    }
...
// ═══════════ Monitor (Live Poll & Topology) ═══════════
// Compatibilidade: usar extractor-status-dot (novo) com fallback para core-status-dot (antigo)
const statusDot = document.getElementById('extractor-status-dot') || document.getElementById('core-status-dot');
const statusLabel = document.getElementById('extractor-status-label') || document.getElementById('core-status-label');
const statusDetail = document.getElementById('extractor-engine-detail') || document.getElementById('core-status-detail');
const vaultHistory = document.getElementById('vault-history');
const accessCount = document.getElementById('access-count');
const liveFeed = document.getElementById('live-feed');
const timeTravelSlider = document.getElementById('time-travel-slider'); // null no novo layout (removido)
const timeTravelLabel = document.getElementById('time-travel-label');   // null no novo layout
...
function appendMonitorLine(text) {
    const liveFeedMonitor = document.getElementById('live-feed-monitor');
    if(!liveFeedMonitor) return;
    
    if(liveFeedMonitor.innerHTML.includes('não iniciado')) {
        liveFeedMonitor.innerHTML = '';
    }
    ...
    div.innerHTML = `
        <div style="display:flex; justify-content:space-between; width: 100%;">
            <span class="row-v" style="color:var(--text-3); font-size:9px;">${new Date().toLocaleTimeString()}</span>
        </div>
        <span style="font-size: 11px; color: var(--green); font-family: var(--mono);">${text}</span>
    `;
    liveFeedMonitor.appendChild(div);
    liveFeedMonitor.parentElement.scrollTop = liveFeedMonitor.parentElement.scrollHeight;
}
...
const monitorPoll = async () => {
    try {
        const r = await fetch(SERVER_URL + '/api/inject-ig/status', { signal: AbortSignal.timeout(3000) });
        if (r.ok) {
            const data = await r.json();
            coreOnline = true;
            if (statusDot) statusDot.style.background = 'var(--green)';
            if (statusLabel) { statusLabel.className = 'tag tag-green'; statusLabel.textContent = 'Online'; }
            
            const gDot = document.getElementById('global-status-dot');
            const gLab = document.getElementById('global-status-label');
            if (gDot) gDot.style.background = 'var(--green)';
            if (gLab) { gLab.className = 'tag tag-green'; gLab.textContent = 'Online'; }
        ...
        // Silencia erro de fetch (Core backend offline é comportamento esperado)
        coreOnline = false;
        if (statusDot) statusDot.style.background = 'var(--red)';
        if (statusLabel) { statusLabel.className = 'tag tag-red'; statusLabel.textContent = 'Offline'; }
        const gDot = document.getElementById('global-status-dot');
        const gLab = document.getElementById('global-status-label');
        if (gDot) gDot.style.background = 'var(--red)';
        if (gLab) { gLab.className = 'tag tag-red'; gLab.textContent = 'Offline'; }
    }
    ...
    try {
        ...
        if (timeTravelSlider && timeTravelLabel) {
...
document.getElementById('btn-start-core')?.addEventListener('click', async () => {
    term.writeln('\r\n\x1b[35m[sys]\x1b[0m Reiniciando e verificando Core Engine em segundo plano...');
    if (window.electronAPI && window.electronAPI.restartBackend) {
        window.electronAPI.restartBackend();
    }
    
    // Testa a conexão após alguns segundos
    setTimeout(async () => {
        try {
            const res = await fetch(SERVER_URL + '/api/inject-ig/status', { signal: AbortSignal.timeout(2000) });
            if (res.ok) {
                term.writeln('\x1b[32m[ok] Motor Core respondendo perfeitamente (Online).\x1b[0m');
                term.writeln('\x1b[32m[ok] A varredura no menu Arquivos está pronta para uso!\x1b[0m');
            } else {
                term.writeln('\x1b[33m[warn] Motor iniciado, mas a API não respondeu. Tente novamente.\x1b[0m');
            }
        } catch(e) {
            term.writeln('\x1b[33m[warn] Motor está iniciando... aguarde alguns segundos antes de varrer.\x1b[0m');
        }
        term.prompt();
    }, 2000);
});
document.getElementById('btn-inject-local')?.addEventListener('click', async () => {
    ...
    if (p) {
        if (window.DI_Engine && window.DI_Engine.showAction) {
            window.DI_Engine && window.DI_Engine.showAction('Injetando...', '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="filter: drop-shadow(0 0 4px rgba(239, 68, 68, 0.4));"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"></polygon></svg>', 3000);
        }
...
function grantAccess(user) {
    ...
    // Show app
    if (mainTitlebar) mainTitlebar.style.display = 'flex';
    ...
    if (profileAvatarMac) profileAvatarMac.src = avatarToUse;
    const profileNameMac = document.getElementById('profile-name-mac');
    if (profileNameMac) profileNameMac.innerText = user.username;
    const userProfileHeaderMac = document.getElementById('user-profile-header-mac');
    if (userProfileHeaderMac) userProfileHeaderMac.style.display = 'flex';
    ...
    if (window.isUpdateAvailable) {
        const updateModal = document.getElementById('modal-update-alert');
...
// ── Payment Logic ──
let selectedPlan = 30; // Default
let selectedPrice = '99.90';

// Change Plans
document.querySelectorAll('.payment-plan-card').forEach(card => {
    card.addEventListener('click', (e) => {
        document.querySelectorAll('.payment-plan-card').forEach(c => c.classList.remove('active'));
        const target = e.currentTarget;
        target.classList.add('active');
        selectedPlan = parseInt(target.getAttribute('data-plan'));
        selectedPrice = target.getAttribute('data-price');
    });
});

// "Já tenho uma chave"
document.getElementById('btn-back-to-license')?.addEventListener('click', () => {
    document.getElementById('view-payment').style.display = 'none';
    document.getElementById('view-license').style.display = 'flex';
});

// Modal de Suporte
document.getElementById('btn-open-suporte')?.addEventListener('click', () => {
    document.getElementById('modal-suporte').style.display = 'flex';
});

document.getElementById('btn-open-suporte-settings')?.addEventListener('click', () => {
    document.getElementById('modal-suporte').style.display = 'flex';
    initSupportUI();
});

document.getElementById('btn-open-suporte-mac')?.addEventListener('click', () => {
    document.getElementById('modal-suporte').style.display = 'flex';
    initSupportUI();
});

document.getElementById('btn-close-suporte')?.addEventListener('click', () => {
    document.getElementById('modal-suporte').style.display = 'none';
});

document.getElementById('btn-go-whatsapp')?.addEventListener('click', () => {
    const msg = encodeURIComponent("Olá! Eu já realizei o pagamento do Inject-IG, mas meu acesso ainda não foi liberado. Segue o comprovante de pagamento em anexo.");
    window.electronAPI.openExternal(`https://wa.me/5531980420304?text=${msg}`);
});

// Suporte UI Logic
const tabReport = document.getElementById('tab-suporte-report');
const tabUpdates = document.getElementById('tab-suporte-updates');
const contentReport = document.getElementById('suporte-content-report');
const contentUpdates = document.getElementById('suporte-content-updates');

tabReport?.addEventListener('click', () => {
    tabReport.classList.add('active');
    tabReport.style.borderBottomColor = 'var(--accent)';
    tabReport.style.color = '#fff';
    
    tabUpdates.classList.remove('active');
    tabUpdates.style.borderBottomColor = 'transparent';
    tabUpdates.style.color = 'var(--text-2)';
    
    contentReport.style.display = 'flex';
    contentUpdates.style.display = 'none';
});

tabUpdates?.addEventListener('click', () => {
    tabUpdates.classList.add('active');
    tabUpdates.style.borderBottomColor = 'var(--accent)';
    tabUpdates.style.color = '#fff';
    
    tabReport.classList.remove('active');
    tabReport.style.borderBottomColor = 'transparent';
    tabReport.style.color = 'var(--text-2)';
    
    contentUpdates.style.display = 'flex';
    contentReport.style.display = 'none';
    
    loadAppUpdates();
});

// Real-time Support Chat & Admin Mode Logic
let isAdminMode = false;
let adminSelectedHwid = null;

function renderMessageBubble(msg, containerId) {
    const container = document.getElementById(containerId);
    if (!container) return;

    // Remove o texto "Carregando" se existir
    if (container.innerHTML.includes('Carregando histórico') || container.innerHTML.includes('Nenhum chat selecionado')) {
        container.innerHTML = '';
    }

    const isMine = isAdminMode ? msg.is_admin : !msg.is_admin;
    const bubbleColor = isMine ? '#007aff' : 'rgba(255,255,255,0.1)';
...
async function loadUserChat() {
    const container = document.getElementById('suporte-user-messages');
    container.innerHTML = '<div style="text-align: center; color: var(--text-3); font-size: 12px; margin-top: auto;">Carregando histórico...</div>';
    
    const res = await window.electronAPI.getSupportMessages(currentHWID);
    if (res && res.success) {
        container.innerHTML = '';
        res.messages.forEach(msg => renderMessageBubble(msg, 'suporte-user-messages'));
    } else {
        container.innerHTML = '<div style="text-align: center; color: var(--red); font-size: 12px; margin-top: auto;">Erro ao carregar histórico.</div>';
    }
}

async function loadAdminChats() {
    const container = document.getElementById('suporte-admin-hwids');
    container.innerHTML = '<div style="padding: 15px; color: var(--text-3); font-size: 12px;">Carregando...</div>';
    
    const res = await window.electronAPI.getAllActiveChats();
    if (res && res.success) {
        container.innerHTML = '';
        res.chats.forEach(chat => {
            ...
            `;
            btn.innerHTML = `
                <div style="font-weight: 600; font-size: 13px; color: #fff; margin-bottom: 4px;">HWID: ${chat.hwid.substring(0, 8)}...</div>
            ...
            btn.onclick = () => selectAdminChat(chat.hwid);
            container.appendChild(btn);
        });
    }
}

async function selectAdminChat(hwid) {
    adminSelectedHwid = hwid;
    document.getElementById('suporte-admin-header').innerHTML = `
        <span>Chat com: ${hwid.substring(0, 12)}...</span>
        <span style="font-size: 11px; color: var(--green);">Conectado</span>
    `;
    ...
    container.innerHTML = '<div style="text-align: center; color: var(--text-3); font-size: 12px; margin: auto;">Carregando mensagens...</div>';
    
    const res = await window.electronAPI.getSupportMessages(hwid);
    if (res && res.success) {
        container.innerHTML = '';
        res.messages.forEach(msg => renderMessageBubble(msg, 'suporte-admin-messages'));
    }
}

function initSupportUI() {
    isAdminMode = false;
    document.getElementById('suporte-user-view').style.display = 'flex';
    if(document.getElementById('suporte-admin-view')) {
        document.getElementById('suporte-admin-view').style.display = 'none';
    }
    loadUserChat();
}

// IPC Realtime Listener
window.electronAPI.onRealtimeSupportMessage((msg) => {
    if (isAdminMode) {
        if (msg.hwid === adminSelectedHwid) {
            renderMessageBubble(msg, 'suporte-admin-messages');
        }
        loadAdminChats(); // Update sidebar latest message
    } else {
        if (msg.hwid === currentHWID) {
            renderMessageBubble(msg, 'suporte-user-messages');
        }
    }
});

// User Chat Form Submit
document.getElementById('suporte-user-form')?.addEventListener('submit', async (e) => {
    e.preventDefault();
    const input = document.getElementById('suporte-user-input');
    const message = input.value.trim();
    if (!message) return;
    
    input.value = '';
    
    // Otimista (opcional, ou espera o realtime voltar)
    // Usaremos apenas o banco via realtime para evitar duplicatas, mas para UI rápida:
    const tempMsg = { message, is_admin: false, created_at: new Date().toISOString() };
    renderMessageBubble(tempMsg, 'suporte-user-messages');
    
    await window.electronAPI.submitSupportTicket(currentHWID, message, false).catch(console.error);
});

// Admin Chat Form Submit
document.getElementById('suporte-admin-form')?.addEventListener('submit', async (e) => {
    e.preventDefault();
    const input = document.getElementById('suporte-admin-input');
    const message = input.value.trim();
    if (!message || !adminSelectedHwid) return;
    
    input.value = '';
    
    const tempMsg = { message, is_admin: true, created_at: new Date().toISOString() };
    renderMessageBubble(tempMsg, 'suporte-admin-messages');
    
    await window.electronAPI.submitSupportTicket(adminSelectedHwid, message, true).catch(console.error);
});
...
async function loadAppUpdates() {
    ...
    try {
        const res = await window.electronAPI.getAppUpdates();
        if (res && res.success) {
            container.innerHTML = '';
            if (res.updates.length === 0) {
                container.innerHTML = '<div style="text-align: center; color: var(--text-3); font-size: 12px; padding: 20px;">Nenhuma atualização recente.</div>';
                return;
            }
            
            res.updates.forEach(upd => {
                const date = new Date(upd.created_at).toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' });
                const icon = upd.type === 'update' 
                    ? '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="17 8 12 3 7 8"></polyline><line x1="12" y1="3" x2="12" y2="15"></line></svg>'
                    : '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--green)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path></svg>';
                
                const title = upd.type === 'update' ? 'Nova Versão Lançada' : 'Mensagem do Administrador';
                    ...
                    <div style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 16px; padding: 16px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                            <div style="display: flex; align-items: center; gap: 8px;">
                                ...
                                <span style="font-weight: 600; color: #fff; font-size: 13px;">${title}</span>
                            </div>
                            <span style="font-size: 11px; color: var(--text-3);">${date}</span>
                        </div>
                        <div style="color: var(--text-2); font-size: 12px; line-height: 1.6; padding-left: 4px; white-space: pre-wrap;">${upd.content}</div>
                    </div>
                `;
            });
        }
    } catch (e) {
        container.innerHTML = '<div style="text-align: center; color: var(--red); font-size: 12px; padding: 20px;">Erro ao carregar atualizações.</div>';
    }
}

// Pay with Mercado Pago (Static Links + Admin Approval)
let pendingPollingInterval = null;
document.getElementById('btn-pay-mp')?.addEventListener('click', async () => {
    const nick = document.getElementById('payment-nick-input').value.trim();
    ...
    // Mercado Pago Static Links
    let url = '';
    if (selectedPlan === 1) url = 'https://mpago.la/1pPc6uc';
    else if (selectedPlan === 30) url = 'https://mpago.la/1agroaq';
    else if (selectedPlan === 90) url = 'https://mpago.la/2EW5a4q';

    if (url) {
        // Register pending payment in Supabase
        const pendingRes = await window.electronAPI.registerPendingPayment(nick, selectedPlan, currentHWID);
        
        if (pendingRes.success && pendingRes.pending_id) {
            // Show waiting screen
            document.getElementById('payment-stripe-checkout').style.display = 'flex';
            document.getElementById('payment-stripe-checkout').querySelector('.card-title').innerText = 'Aguardando Pagamento';
            document.getElementById('stripe-status-text').innerText = 'Por favor, finalize o pagamento no Mercado Pago. O sistema aguarda aprovação.';

            // Open URL in external browser
            window.electronAPI.openExternal(url);
            
            // Start polling for admin approval
            pendingPollingInterval = setInterval(async () => {
                const statusRes = await window.electronAPI.checkPendingStatus(pendingRes.pending_id, currentHWID, selectedPlan);
                if (statusRes.approved) {
                    clearInterval(pendingPollingInterval);
                    document.getElementById('payment-stripe-checkout').style.display = 'none';
                    document.getElementById('payment-success').style.display = 'flex';
                }
            }, 5000); // Check every 5 seconds
        } else {
            errorMsg.innerText = 'Erro ao conectar com o banco de dados. Tente novamente.';
            errorMsg.style.display = 'block';
        }
    }
});
...
// Cancel Polling
document.getElementById('btn-cancel-stripe')?.addEventListener('click', () => {
    if (pendingPollingInterval) clearInterval(pendingPollingInterval);
    document.getElementById('payment-stripe-checkout').style.display = 'none';
});

// Enter system after payment success
document.getElementById('btn-enter-after-pay')?.addEventListener('click', () => {
    // Reload flow to pick up the new active license automatically
    window.location.reload();
});
...
if (window.electronAPI && window.electronAPI.onUpdateStatus) {
    window.electronAPI.onUpdateStatus((msg) => {
        ...
        if (msg.includes('Atualização disponível')) {
            window.isUpdateAvailable = true;
            const badgeSettings = document.getElementById('update-badge-settings');
            const badgeCard = document.getElementById('update-badge-card');
            if (badgeSettings) badgeSettings.style.display = 'block';
...
document.getElementById('btn-update-now')?.addEventListener('click', () => {
    document.getElementById('modal-update-alert').style.display = 'none';
    const settingsBtn = document.querySelector('.seg-btn[data-target="view-settings"]');
...
document.getElementById('btn-pc-scan')?.addEventListener('click', async () => {
    ...
    try {
        ...
        if (res && res.success) {
            let html = `<b>${res.files} Arquivos</b> (${res.sizeMb.toFixed(2)} MB) em <i>${res.path}</i><br><br>`;
            if (res.topFiles && res.topFiles.length > 0) {
                html += `<b>Maiores Arquivos Encontrados:</b><br>`;
                html += res.topFiles.map(f => `• ${f.name} <span style="color:var(--text-3)">(${f.sizeMb})</span>`).join('<br>');
            }
...
// ═══════════ BIGSUR CONSOLE MODE & REACTIVE AUDIO ENGINE ═══════════

let audioCtx = null;
let isBigSurMode = localStorage.getItem('ig_bigsur') === 'true';
...
function playBootSound() {
    ...
    try {
        initAudioContext();
        // PS5 style deep cinematic swoosh
        const osc = audioCtx.createOscillator();
        ...
        osc2.frequency.exponentialRampToValueAtTime(200, audioCtx.currentTime + 2);
...
let consoleCards = [];
let isConsoleAppOpen = false;

function updateConsoleFocus() {
    if (consoleCards.length === 0) {
        consoleCards = Array.from(document.querySelectorAll('.console-card'));
    }
    consoleCards.forEach((card, idx) => {
    ...
    const carousel = document.getElementById('console-carousel');
    if (carousel && consoleCards.length > 0) {
        const cardWidth = 320;
        const gap = 40;
        const offset = - (currentFocusIndex - (consoleCards.length - 1) / 2) * (cardWidth + gap);
...
function handleConsoleKeydown(e) {
    ...
    // Se um app estiver aberto dentro do console, Esc fecha o app e volta pro carrossel.
    if (isConsoleAppOpen) {
        if (e.key === 'Escape') {
            closeConsoleApp();
        }
        return; // Não processa setas se um app estiver dominando a tela
    }

    if (e.key === 'ArrowRight') {
        if (currentFocusIndex < consoleCards.length - 1) {
            currentFocusIndex++;
            playHoverSound();
            updateConsoleFocus();
        }
        ...
        playClickSound();
        const activeCard = consoleCards[currentFocusIndex];
        if (activeCard) {
            const action = activeCard.getAttribute('data-action');
            if (action === 'exit-console') {
                setBigSurMode(false);
            } else if (action && action.startsWith('run-')) {
                const targetViewName = action.replace('run-', 'view-');
                openConsoleApp(targetViewName);
            }
        }
...
function openConsoleApp(targetViewName) {
    const btn = document.querySelector(`.seg-btn[data-target="${targetViewName}"]`);
    if (btn) btn.click(); // Roteamento padrão do sistema para carregar a view

    isConsoleAppOpen = true;
    document.documentElement.classList.add('theme-bigsur-app-active');
    
    const mainApp = document.querySelector('.app');
    const consoleUi = document.getElementById('view-console-ui');
    const hud = document.getElementById('console-hud');
    
    if (mainApp) mainApp.style.display = 'flex';
    if (consoleUi) consoleUi.style.opacity = '0';
    if (hud) hud.classList.add('visible');
    
    // Timeout para ocultar o container após a transição
    setTimeout(() => {
        if (isConsoleAppOpen && consoleUi) {
...
function closeConsoleApp() {
    isConsoleAppOpen = false;
    document.documentElement.classList.remove('theme-bigsur-app-active');
    ...
    if (consoleUi) {
        ...
        // Pequeno delay para a opacidade voltar suavemente
        setTimeout(() => {
            consoleUi.style.opacity = '1';
        }, 50);
    }
}

function setBigSurMode(enabled, playBoot = false) {
    isBigSurMode = enabled;
    localStorage.setItem('ig_bigsur', enabled);
    const toggle = document.getElementById('toggle-bigsur');
    ...
    if (enabled) {
        document.documentElement.classList.add('theme-bigsur');
        if (window.electronAPI && window.electronAPI.setFullscreen) {
            window.electronAPI.setFullscreen(true);
        }
        if (mainApp) mainApp.style.display = 'none';
        if (consoleUi) consoleUi.style.display = 'flex';
        ...
        if (playBoot) setTimeout(playBootSound, 200);
    } else {
        isConsoleAppOpen = false;
        document.documentElement.classList.remove('theme-bigsur');
        document.documentElement.classList.remove('theme-bigsur-app-active');
        const hud = document.getElementById('console-hud');
        if (hud) hud.classList.remove('visible');
        ...
        const consoleCarousel = document.getElementById('console-carousel-container');
        if (consoleCarousel) {
            consoleCarousel.style.display = 'flex';
            consoleCarousel.style.opacity = '1';
        }
...
// Global Event Listeners for Reactive Audio
document.addEventListener('mouseover', (e) => {
    if (!isBigSurMode) return;
    const target = e.target.closest('button, .btn, .seg-btn, .sub-seg-btn, .bento-item, .module-card');
...
// Init
setTimeout(() => {
    const toggleBtn = document.getElementById('toggle-bigsur');
    if (toggleBtn) {
        toggleBtn.addEventListener('change', (e) => {
            setBigSurMode(e.target.checked, true);
        });
    }
    
    // Auto-apply on start
    if (isBigSurMode) {
        setBigSurMode(true, true); // Play boot on app start if enabled
    }
}, 500);

// ═══════════════════════════════════════════════════════════════════
// ── EXTRACTOR ENGINE — Botões e Listeners ─────────────────────────
// ═══════════════════════════════════════════════════════════════════

// Botão "Iniciar Motor" (ícone de play no header)
document.getElementById('btn-start-extractor')?.addEventListener('click', async () => {
    setExtractorStatus('⏳ Iniciando SiteExtractorEngine (Java)...', true);
    try {
        const res = await window.electronAPI.extractorStart();
        setExtractorStatus(res.success ? `✅ ${res.message}` : `❌ ${res.message}`);
        if (res.success) {
            const dot = document.getElementById('extractor-status-dot');
            const lbl = document.getElementById('extractor-status-label');
            if (dot) dot.style.background = 'var(--green)';
            if (lbl) { lbl.textContent = 'Online'; lbl.className = 'tag tag-green'; }
        }
        appendMonitorLine(`[ENGINE] ${res.message}`);
    } catch (e) {
        setExtractorStatus(`❌ ${e.message}`);
    }
});
...
document.getElementById('btn-monitor-start')?.addEventListener('click', async () => {
    ...
    const dot = document.getElementById('extractor-status-dot');
    const lbl = document.getElementById('extractor-status-label');
    const detail = document.getElementById('extractor-engine-detail');
    if (res.success) {
        if (dot) dot.style.background = 'var(--green)';
        if (lbl) { lbl.textContent = 'Online'; lbl.className = 'tag tag-green'; }
        if (detail) detail.textContent = 'Porta 7890 · Online';
    } else {
        if (detail) detail.textContent = `Erro: ${res.message}`;
    }
    appendMonitorLine(`[ENGINE] ${res.message}`);
});

// Botão "Parar" no Monitor
document.getElementById('btn-monitor-stop')?.addEventListener('click', async () => {
    await window.electronAPI.extractorStop().catch(() => {});
    const dot = document.getElementById('extractor-status-dot');
    const lbl = document.getElementById('extractor-status-label');
    const detail = document.getElementById('extractor-engine-detail');
    if (dot) dot.style.background = 'var(--red)';
    if (lbl) { lbl.textContent = 'Offline'; lbl.className = 'tag tag-red'; }
    if (detail) detail.textContent = 'Porta 7890 · Parado';
    appendMonitorLine('[ENGINE] Motor parado pelo usuário.');
});

// Busca em tempo real nos arquivos
...
// Exportar toda a categoria ativa como JSON
document.getElementById('btn-export-category')?.addEventListener('click', async () => {
    if (!currentExtractorData[activeCategory]) return;
    const res = await window.electronAPI.extractorDownloadCategory(
        activeCategory,
        currentExtractorData[activeCategory]
    ).catch(e => ({ success: false, message: e.message }));
    appendMonitorLine(res.success ? `[EXPORT] ${res.message}` : `[EXPORT] ❌ ${res.message}`);
});

// Exportar tudo (Resumo)
document.getElementById('btn-save-report')?.addEventListener('click', async () => {
    if (!Object.keys(currentExtractorData).length) return;
    const res = await window.electronAPI.extractorDownloadCategory(
        'completo',
        currentExtractorData
    ).catch(e => ({ success: false, message: e.message }));
    appendMonitorLine(res.success ? `[EXPORT] Relatório completo: ${res.message}` : `[EXPORT] ❌ ${res.message}`);
});

// Telemetria polling do motor Java a cada 3s
setInterval(async () => {
    try {
        const tel = await window.electronAPI.extractorTelemetry();
        if (!tel) return;
        const ramBar = document.getElementById('telemetry-ram-bar');
        const ramLabel = document.getElementById('telemetry-ram-label');
        const threads = document.getElementById('telemetry-threads');
        const vault = document.getElementById('telemetry-vault');
        if (ramBar && tel.heapMax > 0) {
            const pct = Math.round((tel.heapUsed / tel.heapMax) * 100);
            ramBar.style.width = pct + '%';
            ramBar.style.background = pct > 80 ? 'var(--red)' : pct > 50 ? 'var(--amber)' : 'var(--accent)';
        }
        if (ramLabel) ramLabel.textContent = `${tel.heapUsed || 0} MB / ${tel.heapMax || 0} MB`;
        if (threads) threads.textContent = tel.threadsActive || 0;
        if (vault) vault.textContent = tel.totalExtracted || 0;
        const intercepts = document.getElementById('telemetry-intercepts');
        if (intercepts && tel.lastTarget) intercepts.textContent = tel.lastTarget;
        const dot = document.getElementById('extractor-status-dot');
        const lbl = document.getElementById('extractor-status-label');
        if (dot) dot.style.background = 'var(--green)';
        if (lbl && lbl.textContent === 'Offline') {
            lbl.textContent = 'Online'; lbl.className = 'tag tag-green';
        }
    } catch (e) { /* engine offline */ }
}, 3000);

// Verifica health ao entrar no Monitor
document.getElementById('tab-monitor')?.addEventListener('click', async () => {
    const health = await window.electronAPI.extractorHealth().catch(() => ({ online: false }));
    const dot = document.getElementById('extractor-status-dot');
    const lbl = document.getElementById('extractor-status-label');
    const detail = document.getElementById('extractor-engine-detail');
    if (health.online) {
        if (dot) dot.style.background = 'var(--green)';
        if (lbl) { lbl.textContent = 'Online'; lbl.className = 'tag tag-green'; }
        if (detail) detail.textContent = `Porta 7890 · uptime: ${Math.round((health.data?.uptime || 0) / 1000)}s`;
    } else {
        if (dot) dot.style.background = 'var(--red)';
        if (lbl) { lbl.textContent = 'Offline'; lbl.className = 'tag tag-red'; }
        if (detail) detail.textContent = 'Porta 7890 · Aguardando';
    }
});

// Atualiza aba Arquivos ao clicar
document.getElementById('tab-files')?.addEventListener('click', () => {
    if (Object.keys(currentExtractorData).length > 0) renderExtractorFiles(currentExtractorData);
});
